...
var bust_interp_max_scale:float = 1.0
...
func _ready():
	...
	elif Util.interp_mode: #added for interpolation mode
		coomer.show()
		bust_goal = 1
...
func update_bust():
	...
	if Util.interp_mode:
		...
		match current_bust:
			FRUIT_TYPE.FOURTH:
				print_debug("INTERPOLATION_FOURTH")
				bust_goal = 1
			FRUIT_TYPE.FIFTH:
				print_debug("INTERPOLATION_FIFTH")
				bust_goal = 1
			FRUIT_TYPE.SIXTH:
				print_debug("INTERPOLATION_SIXTH")
				bust_goal = 1
			FRUIT_TYPE.SEVENTH:
				print_debug("INTERPOLATION_SEVENTH")
				bust_goal = 1
			FRUIT_TYPE.EIGHTH:
				print_debug("INTERPOLATION_EIGHTH")
				bust_goal = 2
				bust_interp_max_scale = 1.09
			FRUIT_TYPE.NINTH:
				print_debug("INTERPOLATION_NINTH")
				bust_goal = 3
				bust_interp_max_scale = 1.1
			FRUIT_TYPE.TENTH:
				print_debug("INTERPOLATION_TENTH")
				bust_goal = 4
				bust_interp_max_scale = 1.1
			FRUIT_TYPE.ELEVENTH:
				print_debug("INTERPOLATION_ELEVENTH")
				bust_goal = 5
				bust_interp_max_scale = 1.1
			FRUIT_TYPE.TWELFTH:
				print_debug("INTERPOLATION_TWELFTH")
				bust_goal = 6
				bust_interp_max_scale = 1.1
		
		$Avatar/GirlBreasts.scale = Vector2(interpolateScale(float(bust_amount), 0, float(bust_goal), float(1), float(bust_interp_max_scale)), interpolateScale(float(bust_amount), 0, float(bust_goal), float(1), float(bust_interp_max_scale)))
		
		print("scale:" + str($Avatar/GirlBreasts.scale))
...
func _on_increase_bust_pressed():
	update_bust()




func _on_reset_bust_pressed():
	reset_bust()
